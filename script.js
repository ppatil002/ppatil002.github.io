...
let LIST = [],
  id = 0,    //indexes of elements added in list
  count_for_cb = 1000;  //Deleting 1000 entries of the list

function addToDo(toDo, id, cb, rd1, rd2, rd3) {
  if (rd1.checked) {
    // color=red;
...
function addtask() {
  ...
  var a = task.value;
  if (flag == 1) {
    addToDo(a, id, count_for_cb, rd1, rd2, rd3);
    LIST.push({ name: a, id: id, count_for_cb: count_for_cb });
    id++;
    count_for_cb++;
  }
  task.value = "";
